...
uint32_t CAN_ID=0;
extern double Set_Position;
extern double Position;
extern int32_t Rotary, Counter;
/*
tin hieu sethome 0x100
tin hieu sethome xong 0x111
tin hieu nhan goc 0x201
tin hieu error 0x010
*/
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
  ...
  CAN_ID=RxMessage.StdId;
	...
	if(CAN_ID == 0x201) // receive position
	{
		TIM_Cmd(TIM2,ENABLE);
	...
	else if(CAN_ID == 0x111) //set home complete
	{
...
{
	/*
	mode reponse send position ---> ID = 0x301 (motor1) data = 5 byte
	*/
	CanTxMsg TxMessage;
